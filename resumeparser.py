...
import os
import re
from src.python_utils import cleanText
import nltk
...
class Resumeparser(object):
    ...
    def init():
        Resumeparser.ready = False
        print("loading nltk data", file=sys.stderr)
        nltk.download('punkt')
        ...
        Resumeparser.base_path = os.path.dirname(__file__)

        print("loading en lang model", file=sys.stderr)
        Resumeparser.nlp = spacy.load('en_core_web_sm')
        print("loading degree model", file=sys.stderr)
        Resumeparser.custom_nlp2 = spacy.load(
            os.path.join(Resumeparser.base_path, "degree", "model"))
        print("loading company model", file=sys.stderr)
        Resumeparser.custom_nlp3 = spacy.load(os.path.join(
            Resumeparser.base_path, "company_working", "model"))

        # initialize matcher with a vocab
        print("loading matcher", file=sys.stderr)
        Resumeparser.matcher = Matcher(Resumeparser.nlp.vocab)

        print("loading combined title list", file=sys.stderr)
        file = os.path.join(Resumeparser.base_path, "titles_combined.txt")
        file = open(file, "r", encoding='utf-8')
        designation = [line.strip().lower() for line in file]
        Resumeparser.designitionmatcher = PhraseMatcher(Resumeparser.nlp.vocab)
        patterns = [Resumeparser.nlp.make_doc(text)
                    for text in designation if len(Resumeparser.nlp.make_doc(text)) < 10]
        Resumeparser.designitionmatcher.add("Job title", None, *patterns)
        file.close()

        print("loading skill list", file=sys.stderr)
        file = os.path.join(Resumeparser.base_path,
                            "LINKEDIN_SKILLS_ORIGINAL.txt")
        file = open(file, "r", encoding='utf-8')
        skill = [line.strip().lower() for line in file]
        Resumeparser.skillsmatcher = PhraseMatcher(Resumeparser.nlp.vocab)
        patterns = [Resumeparser.nlp.make_doc(text)
                    for text in skill if len(Resumeparser.nlp.make_doc(text)) < 10]
        Resumeparser.skillsmatcher.add("Job title", None, *patterns)
        print("resume parser ready", file=sys.stderr)
        file.close()
        Resumeparser.ready = True
    ...
    def find_segment_indices(string_to_search, resume_segments, resume_indices):
        for i, line in enumerate(string_to_search):
            ...
            if [o for o in Resumeparser.objective if header.startswith(o)]:
                try:
                    resume_segments['objective'][header]
                except:
                    resume_indices.append(i)
                    header = [
                        o for o in Resumeparser.objective if header.startswith(o)][0]
                    resume_segments['objective'][header] = i
            elif [w for w in Resumeparser.work_and_employment if header.startswith(w)]:
                try:
                    resume_segments['work_and_employment'][header]
                except:
                    resume_indices.append(i)
                    header = [
                        w for w in Resumeparser.work_and_employment if header.startswith(w)][0]
                    resume_segments['work_and_employment'][header] = i
            elif [e for e in Resumeparser.education_and_training if header.startswith(e)]:
                try:
                    resume_segments['education_and_training'][header]
                except:
                    resume_indices.append(i)
                    header = [
                        e for e in Resumeparser.education_and_training if header.startswith(e)][0]
                    resume_segments['education_and_training'][header] = i
            elif [s for s in Resumeparser.skills_header if header.startswith(s)]:
                try:
                    resume_segments['skills'][header]
                except:
                    resume_indices.append(i)
                    header = [
                        s for s in Resumeparser.skills_header if header.startswith(s)][0]
                    resume_segments['skills'][header] = i
            elif [m for m in Resumeparser.misc if header.startswith(m)]:
                try:
                    resume_segments['misc'][header]
                except:
                    resume_indices.append(i)
                    header = [
                        m for m in Resumeparser.misc if header.startswith(m)][0]
                    resume_segments['misc'][header] = i
            elif [a for a in Resumeparser.accomplishments if header.startswith(a)]:
                try:
                    resume_segments['accomplishments'][header]
                except:
                    resume_indices.append(i)
                    header = [
                        a for a in Resumeparser.accomplishments if header.startswith(a)][0]
    ...
    def get_experience(resume_segments):
        ...
        if len(resume_segments['work_and_employment'].keys()):
            ...
            for key, values in resume_segments['work_and_employment'].items():
                text += " ".join(values) + " "
            total_exp = Resumeparser.calculate_experience(text)
    ...
    def extract_name(resume_text):
        nlp_text = Resumeparser.nlp(resume_text)

        # First name and Last name are always Proper Nouns
        # pattern_FML = [{'POS': 'PROPN', 'ENT_TYPE': 'PERSON', 'OP': '+'}]

        pattern = [{'POS': 'PROPN'}, {'POS': 'PROPN'}]
        Resumeparser.matcher.add('NAME', None, pattern)

        matches = Resumeparser.matcher(nlp_text)
    ...
    def job_designition(text):
        ...
        __nlp = Resumeparser.nlp(text.lower())

        matches = Resumeparser.designitionmatcher(__nlp)
        for match_id, start, end in matches:
    ...
    def get_degree(text):
        doc = Resumeparser.custom_nlp2(text)
        degree = []

        degree = [ent.text.replace("\n", " ") for ent in list(
            doc.ents) if ent.label_ == 'Degree']
        return list(dict.fromkeys(degree).keys())

    def get_company_working(text):
        doc = Resumeparser.custom_nlp3(text)
        company = []

        company = [ent.text.replace("\n", " ") for ent in list(doc.ents)]
        return list(dict.fromkeys(company).keys())
    ...
    def extract_skills(text):
        ...
        matches = Resumeparser.skillsmatcher(__nlp)
        for match_id, start, end in matches:
            span = __nlp[start:end]
            span.text = re.sub(r'[^\w\s]', '', span.text)
    ...
    def read_text(lines):
        ...
        resume_segments = Resumeparser.segment(lines)
        full_text = " ".join(lines)

        email = Resumeparser.extract_email(full_text)
        phone = Resumeparser.find_phone(full_text)
        name = Resumeparser.extract_name(
            " ".join(resume_segments['contact_info']))
        total_exp, text = Resumeparser.get_experience(resume_segments)
        university = Resumeparser.extract_university(
            full_text, os.path.join(Resumeparser.base_path, 'world-universities.csv'))

        designition = Resumeparser.job_designition(full_text)
        designition = list(dict.fromkeys(designition).keys())

        degree = Resumeparser.get_degree(full_text)
        company_working = Resumeparser.get_company_working(full_text)

        skills = ""

        if len(resume_segments['skills'].keys()) > 0:
            for key, values in resume_segments['skills'].items():
                cleanSkill = cleanText(re.sub(key, '', ",".join(values), flags=re.IGNORECASE),
                                       filternum=True, filterstops=True, removelinks=True, removespecial=True)
                if cleanSkill != '':
                    skills += cleanSkill

            skills = [skill for skill in skills.strip().strip(",").split(",")]
